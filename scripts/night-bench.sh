...
# 2. run
cd $LAB
ls ~/data/optopt/miplib/inst | sed 's/\.mps\.gz$//' > bench/sets/miplib-bench-all.txt
python3 bench/run.py --arms bench/arms.toml --only-arms main dev dev-tideseed --set bench/sets/miplib-bench-all.txt \
